...
function POSScreen({ products, setProducts, sailors, setSailors,
  ...
  const categories: Category[] = ["ALL", "LIQUOR", "SOFT DRINKS", "FOOD", "PARTY"];
  ...
  const subCategories = useMemo(() => {
    let subs: string[] = [];
    if (activeCat === "ALL") {
      const fromMaster = (masterCategories || []).flatMap((c: any) => (c.subCategories || []).map((s: any) => s.name));
      const fromProds = products.map((p: any) => p.sub).filter(Boolean);
      subs = Array.from(new Set([...fromMaster, ...fromProds]));
    } else {
      const masterCat = (masterCategories || []).find((c: any) => c.name.toUpperCase() === activeCat.toUpperCase());
      const fromMaster = masterCat ? (masterCat.subCategories || []).map((s: any) => s.name) : [];
      const fromProds = products.filter((p: any) => p.category === activeCat).map((p: any) => p.sub).filter(Boolean);
      subs = Array.from(new Set([...fromMaster, ...fromProds]));
    }
    return ["ALL", ...subs];
  }, [activeCat, products, masterCategories]);

  const brandsList = useMemo(() => {
    const relProds = products.filter((p: any) => {
      const pCat = (p.category || "").trim().toUpperCase();
      const pSub = (p.sub || "").trim().toUpperCase();
      let matchCat = activeCat === "ALL" || pCat === activeCat.toUpperCase();
      if (activeCat === "PARTY") matchCat = true;
      let matchSub = activeSubCat === "ALL";
      if (!matchSub) {
        const selSub = activeSubCat.toUpperCase();
        if (selSub === "PARTY DRINKS") matchSub = pCat === "LIQUOR" || pCat === "SOFT DRINKS" || pSub.includes("DRINK") || pSub === "WHISKY" || pSub === "BEER" || pSub === "RUM" || pSub === "VODKA" || pSub === "WINE" || pSub === "BRANDY" || pSub === "COLD DRINK" || pSub === "PARTY DRINKS";
        else if (selSub === "PARTY FOOD") matchSub = pCat === "FOOD" || pCat === "PARTY FOOD" || pSub.includes("FOOD") || pSub.includes("VEG") || pSub.includes("BREAD") || pSub === "VEGETARIAN" || pSub === "NON VEGETARIAN" || pSub === "BREAD/ROTI";
        else matchSub = pSub === selSub;
      }
      return matchCat && matchSub;
    });

    const extractedBrands = relProds.map((p: any) => {
      if (p.brand) return p.brand;
      if (p.subSub) return p.subSub;
      const nameUpper = (p.name || "").toUpperCase();
      if (nameUpper.includes("ROYAL STAG")) return "ROYAL STAG";
      if (nameUpper.includes("BACARDI")) return "BACARDI";
      if (nameUpper.includes("MCDOWELL")) return "MCDOWELL";
      if (nameUpper.includes("OLD MONK")) return "OLD MONK";
      if (nameUpper.includes("CARLSBERG")) return "CARLSBERG";
      if (nameUpper.includes("KINGFISHER")) return "KINGFISHER";
      if (nameUpper.includes("SMIRNOFF")) return "SMIRNOFF";
      if (nameUpper.includes("BLACK DOG")) return "BLACK DOG";
      if (nameUpper.includes("ANTIQUITY")) return "ANTIQUITY";
      if (nameUpper.includes("CHIVAS")) return "CHIVAS";
      if (nameUpper.includes("JACK DANIEL")) return "JACK DANIEL";
      if (nameUpper.includes("ABSOLUT")) return "ABSOLUT";
      if (nameUpper.includes("HEINEKEN")) return "HEINEKEN";
      if (nameUpper.includes("PEPSI")) return "PEPSI";
      if (nameUpper.includes("7 UP") || nameUpper.includes("7UP")) return "7 UP";
      if (nameUpper.includes("MIRINDA")) return "MIRINDA";
      if (nameUpper.includes("RED BULL")) return "RED BULL";
      if (nameUpper.includes("COCA COLA") || nameUpper.includes("COKE")) return "COCA COLA";
      if (nameUpper.includes("SPRITE")) return "SPRITE";
      return null;
    }).filter(Boolean);

    const fromMaster = (masterBrands || [])
      .filter((b: any) => !b.categoryName || activeCat === "ALL" || b.categoryName.toUpperCase() === activeCat.toUpperCase())
      .map((b: any) => b.name);

    const combined = Array.from(new Set([...extractedBrands, ...fromMaster]));
    return ["ALL", ...combined];
  }, [products, activeCat, activeSubCat, masterBrands]);

  const filtered = useMemo(() => {
    return products.filter((p: any) => {
      const pCat = (p.category || "").trim().toUpperCase();
      const pSub = (p.sub || "").trim().toUpperCase();
      const pBrand = (p.brand || p.subSub || "").trim().toUpperCase();
      const pName = (p.name || "").trim().toUpperCase();

      // 1. Category Matching
      let matchCat = activeCat === "ALL" || pCat === activeCat.toUpperCase();
      if (activeCat === "PARTY") {
        matchCat = pCat === "PARTY" || pCat === "PARTY FOOD" || pCat === "LIQUOR" || pCat === "SOFT DRINKS" || pCat === "FOOD";
      }
      ...
      if (!matchSub) {
        const selSub = activeSubCat.toUpperCase();
        if (selSub === "PARTY DRINKS") {
          matchSub = pCat === "LIQUOR" || pCat === "SOFT DRINKS" || pSub.includes("DRINK") || pSub === "WHISKY" || pSub === "BEER" || pSub === "RUM" || pSub === "VODKA" || pSub === "WINE" || pSub === "BRANDY" || pSub === "COLD DRINK" || pSub === "PARTY DRINKS";
        } else if (selSub === "PARTY FOOD") {
          matchSub = pCat === "FOOD" || pCat === "PARTY FOOD" || pSub.includes("FOOD") || pSub.includes("VEG") || pSub.includes("BREAD") || pSub === "VEGETARIAN" || pSub === "NON VEGETARIAN" || pSub === "BREAD/ROTI";
        } else {
          matchSub = pSub === selSub;
        }
      }
      ...
      if (!matchBrand) {
        const selBrand = activeBrand.toUpperCase();
        if (selBrand === "SOFT DRINKS") {
          matchBrand = pCat === "SOFT DRINKS" || pSub === "COLD DRINK" || pSub === "ENERGY DRINK" || pBrand === "SOFT DRINKS";
        } else if (selBrand === "HARD DRINKS") {
          matchBrand = pCat === "LIQUOR" || ["WHISKY", "BEER", "RUM", "VODKA", "WINE", "BRANDY"].includes(pSub) || pBrand === "HARD DRINKS";
        } else {
          matchBrand = pBrand.includes(selBrand) || pName.includes(selBrand);
        }
      }

      return matchBrand;
    });
  }, [products, activeCat, activeSubCat, activeBrand]);
    ...
    <div className="flex flex-col h-full gap-0">
      ...
      <div className="flex items-center gap-1 px-2 py-1.5 bg-[#061830] border-b border-white/20 flex-shrink-0">
        ...
        <div className="flex items-center gap-1 overflow-x-auto no-scrollbar flex-1">
          {categories.map(c => (
            <button key={c} onClick={() => { setActiveCat(c); setActiveSubCat("ALL"); setActiveBrand("ALL"); }}
        ...
        <button className="text-white/60 hover:text-white ml-1 flex-shrink-0"><ChevronRight size={16} /></button>
      </div>
        ...
        <div className="flex items-center gap-1 overflow-x-auto no-scrollbar flex-1">
          {brandsList.map(b => (
            <button key={b} onClick={() => setActiveBrand(b)}
              style={{ backgroundColor: activeBrand === b ? "#d97706" : "rgba(255,255,255,0.1)" }}
              className={`px-3 py-0.5 rounded text-xs font-semibold transition-all hover:opacity-90 whitespace-nowrap ${activeBrand === b ? "text-white ring-1 ring-amber-300 font-bold" : "text-white/80"
                }`}>
              {b}
            </button>
          ))}
      ...
      <div className="flex flex-1 min-h-0 gap-2 p-2">
        ...
        <div className="w-64 flex-shrink-0 flex flex-col gap-1.5 overflow-y-auto pr-1">
          <div className="text-white text-xs font-bold text-center mb-1">
            Products &nbsp;<span className="bg-white/20 px-2 py-0.5 rounded text-[10px]">{activeCat}{activeSubCat !== "ALL" ? ` · ${activeSubCat}` : ""}{activeBrand !== "ALL" ? ` · ${activeBrand}` : ""}</span>
          </div>
        ...
        {/* Right side panel: Card Payment + Live Stock */}
            ...
            {popupSailor && (
              <div className={`rounded p-2 flex items-center gap-2 text-xs ${popupSailor.status === "Active" ? "bg-sky-950/60 border border-cyan-500/40" : "bg-red-950/60 border border-red-500/40"
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-white font-bold text-xs truncate">{popupSailor.name}</p>
                  <p className="text-white/60 text-[10px]">{popupSailor.rank} · {popupSailor.unit}</p>
                </div>
...
function ProductDetails({ products, setProducts, categories = [], brands = [], onNavigateTo }: { products: any[]; setProducts: any; categories?: any[]; brands?: any[]; onNavigateTo: (s: Screen) => void }) {
  ...
  const categoryOptions = categories && categories.length > 0
    ...
    : ["LIQUOR", "FOOD", "SOFT DRINKS", "PARTY", "SNACKS"];
      ...
      category === "LIQUOR" ? ["WHISKY", "BEER", "RUM", "VODKA", "WINE", "BRANDY"] :
        category === "FOOD" ? ["BREAD/ROTI", "VEGETARIAN", "NON VEGETARIAN", "DESSERTS"] :
          category === "SOFT DRINKS" ? ["COLD DRINK", "ENERGY DRINK"] :
            category === "PARTY" ? ["PARTY DRINKS", "PARTY FOOD"] :
              ["GENERAL"]
    );
  ...
  const brandOptions = brands && brands.length > 0
    ...
    : (
      subCategory === "PARTY DRINKS" ? ["SOFT DRINKS", "HARD DRINKS"] :
        subCategory === "PARTY FOOD" ? ["VEGETARIAN", "NON VEGETARIAN", "BREAD/ROTI"] :
  ...
  function handleCategoryChange(newCat: string) {
    ...
    else if (newCat === "PARTY") subs = ["PARTY DRINKS", "PARTY FOOD"];
    else subs = ["GENERAL"];
...
function AppShell({ currentUser, onLogout }: { currentUser: { username: string; role: string }; onLogout: () => void }) {
  ...
  const [categories, setCategories] = useState<any[]>([
    ...
    {
      id: 4, code: "CAT-04", name: "PARTY", description: "Party Drinks & Catering Supplies", subCategories: [
        { id: 10, categoryId: 4, categoryName: "PARTY", name: "PARTY DRINKS", description: "Soft & Hard Drinks for Parties" },
        { id: 11, categoryId: 4, categoryName: "PARTY", name: "PARTY FOOD", description: "Party Food Platters & Catering" }
      ]
    }
  ]);
  ...
  const [brands, setBrands] = useState<any[]>([
    ...
    { id: 5, name: "SOFT DRINKS", categoryName: "PARTY", subCategoryName: "PARTY DRINKS", description: "Party Soft Drinks" },
    { id: 6, name: "HARD DRINKS", categoryName: "PARTY", subCategoryName: "PARTY DRINKS", description: "Party Hard Drinks" },
    { id: 7, name: "VEGETARIAN", categoryName: "PARTY", subCategoryName: "PARTY FOOD", description: "Party Veg Platters" },
    { id: 8, name: "NON VEGETARIAN", categoryName: "PARTY", subCategoryName: "PARTY FOOD", description: "Party Non-Veg Platters" },
    ...
    async function loadLiveData() {
      try {
        ...
        if (prods && prods.length > 0) {
          const normProds = prods.map((p: any) => {
            if (p.category === "PARTY FOOD") return { ...p, category: "PARTY", sub: "PARTY FOOD" };
            if (p.category === "PARTY" && (p.sub === "PARTY VEG" || p.sub === "PARTY NONVEG")) return { ...p, sub: "PARTY FOOD" };
            return p;
          });
        ...
        if (cats && cats.length > 0) {
          const normCats = cats.map((c: any) => {
            if (c.name === "PARTY FOOD" || c.name === "PARTY") {
              return {
                ...c,
                name: "PARTY",
                subCategories: [
                  { id: 10, categoryId: c.id, categoryName: "PARTY", name: "PARTY DRINKS", description: "Soft & Hard Drinks" },
                  { id: 11, categoryId: c.id, categoryName: "PARTY", name: "PARTY FOOD", description: "Party Veg & Non-Veg Food" },
                ]
              };
            }
            return c;
